...
public class MedicalBillReviewSolutionsPage extends SafeActions implements MedicalBillReviewSolutionLocators {
    ...
    public void verifyImageIsDownloaded(String docId){
        verifyFileDownloaded(docId);
    }

    public void verifyImageIsOpened(){
        String pageSource = getPageSource();
        if(pageSource.contains("AISImgViewer")){
            System.out.println("Image is opened in the new tab");
        } else {
            Assert.fail("Image is not opened after clicking on image");
        }
    }
